document.write([
  '<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBvHsChL4VbPwunZKcj1-i3GqtHan9CYuk&language=en"></script>',
  '<script src="https://ajax.googleapis.com/ajax/libs/angularjs/1.6.4/angular.min.js"></script>',
  '<script src="app.js"></script>',
  '<script src="util/util.services.js"></script>',
  '<script src="search-header/searchHeader.component.js"></script>',
  '<script src="dashboard/dashboard.component.js"></script>',
  '<script src="dashboard/dashboard.mapview.js"></script>',
  '<script src="new-object/newObject.component.js"></script>',
  '<script src="settings/settings.component.js"></script>',
  '<script src="list-view/listView.component.js"></script>',
  '<script src="main-navigation/mainNavigation.component.js"></script>',
  '<script src="login/login.component.js"></script>'
].join('\n'));
